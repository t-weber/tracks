	...
	: QDialog(parent)
{
	...
	connect(m_plot/*.get()*/, &QCustomPlot::mouseMove, this, &DistancesDlg::PlotMouseMove);
	connect(m_plot/*.get()*/, &QCustomPlot::mousePress, this, &DistancesDlg::PlotMouseClick);
	...
	connect(actionSavePdf, &QAction::triggered, this, &DistancesDlg::SavePlotPdf);
	...
	connect(m_all_tracks.get(), &QCheckBox::toggled, this, &DistancesDlg::PlotDistances);
	...
	connect(m_cumulative.get(), &QCheckBox::toggled, this, &DistancesDlg::PlotDistances);

	// plot reset button
	QPushButton *btn_replot = new QPushButton(plot_panel);
	btn_replot->setText("Reset Plot");
	connect(btn_replot, &QAbstractButton::clicked, this, &DistancesDlg::ResetDistPlotRange);
	...
	if(settings.contains("dlg_distances/all_tracks"))
	...
	// table column widths
	if(settings.contains("dlg_distances/date_col"))
		m_table->setColumnWidth(TAB_DATE, settings.value("dlg_distances/date_col").toInt());
	else
		m_table->setColumnWidth(TAB_DATE, 150);

	if(settings.contains("dlg_distances/count_col"))
		m_table->setColumnWidth(TAB_COUNT, settings.value("dlg_distances/count_col").toInt());
	else
		m_table->setColumnWidth(TAB_COUNT, 100);

	if(settings.contains("dlg_distances/dist_col"))
		m_table->setColumnWidth(TAB_DIST, settings.value("dlg_distances/dist_col").toInt());
	else
		m_table->setColumnWidth(TAB_DIST, 150);

	if(settings.contains("dlg_distances/distsum_col"))
		m_table->setColumnWidth(TAB_DIST_SUM, settings.value("dlg_distances/distsum_col").toInt());
	else
		m_table->setColumnWidth(TAB_DIST_SUM, 150);

	if(settings.contains("dlg_distances/time_col"))
		m_table->setColumnWidth(TAB_TIME, settings.value("dlg_distances/time_col").toInt());
	else
		m_table->setColumnWidth(TAB_TIME, 150);

	if(settings.contains("dlg_distances/timesum_col"))
		m_table->setColumnWidth(TAB_TIME_SUM, settings.value("dlg_distances/timesum_col").toInt());
	else
		m_table->setColumnWidth(TAB_TIME_SUM, 150);

	if(settings.contains("dlg_distances/recent_pdfs"))
		m_pdfdir = settings.value("dlg_distances/recent_pdfs").toString().toStdString();
}


DistancesDlg::~DistancesDlg()
{
}


void DistancesDlg::SetTrackDB(const t_tracks *trackdb)
{
	m_trackdb = trackdb;
	CalcDistances();
}
...
void DistancesDlg::FillDistancesTable()
{
...
void DistancesDlg::CalcDistances()
{
	if(!m_trackdb)
		return;
...
void DistancesDlg::ResetDistPlotRange()
{
...
void DistancesDlg::PlotDistances()
{
	if(!m_trackdb || !m_plot)
		return;
 ...
 */
void DistancesDlg::PlotMouseMove(QMouseEvent *evt)
{
	if(!m_plot)
		return;
 ...
 */
void DistancesDlg::PlotMouseClick(QMouseEvent *evt)
{
	if(!m_plot || !m_context)
		return;
...
void DistancesDlg::accept()
{
	...
	settings.setValue("dlg_distances/split", split);

	if(m_table)
	{
		// table column widths
		settings.setValue("dlg_distances/date_col", m_table->columnWidth(TAB_DATE));
		settings.setValue("dlg_distances/count_col", m_table->columnWidth(TAB_COUNT));
		settings.setValue("dlg_distances/dist_col", m_table->columnWidth(TAB_DIST));
		settings.setValue("dlg_distances/distsum_col", m_table->columnWidth(TAB_DIST_SUM));
		settings.setValue("dlg_distances/time_col", m_table->columnWidth(TAB_TIME));
		settings.setValue("dlg_distances/timesum_col", m_table->columnWidth(TAB_TIME_SUM));
	}
